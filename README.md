...
Все постоянные изменения хранятся в ветке `main`.

Для каждой отдельной задачи необходимо создавать временную рабочую ветку от актуального состояния `main`:
...
Затем необходимо создать Pull Request в ветку `main`.

После слияния Pull Request рабочая ветка удаляется.
