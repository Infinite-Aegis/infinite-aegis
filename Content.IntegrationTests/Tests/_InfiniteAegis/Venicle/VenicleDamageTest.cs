...
[TestFixture]
[TestOf(typeof(VenicleDamageComponent))]
public sealed class VenicleDamageTest : GameTest
{
    ...
    public async Task MoskvichDamageStagesAndNoDestruction()
    {
        ...
        await server.WaitAssertion(() =>
        {
            ...
            var venicleDamage = entityManager.GetComponent<VenicleDamageComponent>(moskvich);
            ...
            Assert.Multiple(() =>
            {
                ...
                Assert.That(venicleDamage.MaximumDamage, Is.EqualTo(500f));
            });
            AssertStage(venicleDamage, VenicleDamageState.Normal, 1f, 1f, true);
            ...
            AssertStage(venicleDamage, VenicleDamageState.Damaged20, 0.9f, 1f, true);

            damageableSystem.ChangeDamage(moskvich, new DamageSpecifier(structural, 100), true);
            AssertStage(venicleDamage, VenicleDamageState.Damaged40, 0.8f, 1f, true);

            damageableSystem.ChangeDamage(moskvich, new DamageSpecifier(structural, 100), true);
            AssertStage(venicleDamage, VenicleDamageState.Damaged60, 0.7f, 0.9f, true);

            damageableSystem.ChangeDamage(moskvich, new DamageSpecifier(structural, 100), true);
            AssertStage(venicleDamage, VenicleDamageState.Damaged80, 0.6f, 0.8f, true);

            damageableSystem.ChangeDamage(moskvich, new DamageSpecifier(structural, 100), true);
            AssertStage(venicleDamage, VenicleDamageState.Disabled, 0f, 0f, false);

            var fuelTank = entityManager.GetComponent<VenicleFuelTankComponent>(moskvich);
            ...
            var movementSystem = systemManager.GetEntitySystem<VenicleMovementSystem>();
    ...
    private static void AssertStage(
        VenicleDamageComponent component,
        VenicleDamageState state,
        float acceleration,
        float maximumSpeed,
        bool canDrive)
    {
